...
export function PortfolioProvider({ children }: { children: React.ReactNode }) {
  ...
  // 目前市場報價（用來為持倉估價），啟動時抓一次，之後可用 refreshPrices() 手動更新
  const [prices, setPrices] = useState<Record<string, ETF>>({});
  const [pricesLoading, setPricesLoading] = useState(true);

  const refreshPrices = useCallback(async (forceRefresh = true) => {
    setPricesLoading(true);
    try {
      const all = await fetchAllETFs(forceRefresh);
      const map: Record<string, ETF> = {};
      all.forEach((e) => {
        map[e.symbol] = e;
      });
      setPrices(map);
  ...
  // 依交易紀錄計算目前持倉（加權平均成本法），市價取自即時抓取的 prices
